...
#message
box_1 = {'facecolor':'none',    #  цвет области
...
Radiusmin=20
Radiusmax=20
MaxNum= int( math.pi * Radiusmax * Radiusmax / 1 ) # number persons in square 
...
for i in range(n):
	...
	circle = plt.Circle((items[i][0], items[i][1]), items[i][2], color=fillcolor,fill=True, alpha = 0.5)
	ax.add_artist(circle)
	plt.text(items[i][0]-4, items[i][1]+1,str(items[i][3])+'>'+str(items[i][4]),bbox = box_1,color = 'black',
        fontsize = 9)
plt.text( -10,-25,'итого(нечеткое):'+str(int(FuzzyTotal))+'> итого(четкое):'+str(int(ClearTotal))+' ('+str( int((ClearTotal-FuzzyTotal)/FuzzyTotal * 100) )+'%)',bbox = box_1,color = 'black',  fontsize = 12)       
plt.xlim([-10, 120])
plt.ylim([-10, 120])
...
s = plt.scatter(s_x,s_y,c = s_x,s=0,cmap='jet',alpha=0.4)
c = plt.colorbar()
plt.show()
