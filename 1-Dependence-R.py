...
#
plt.figure(1)
plt.title("Случайное начальное расположение") 
plt.xlabel("R - радиус, 1, 10, 20")   
plt.ylabel("")   
plt.grid()       
#
n =15
...
for i in range(n):
	...
	circle = plt.Circle((items[i][0], items[i][1]), 1, color=fillcolor,fill=True, alpha = 0.3)	
	circle1 = plt.Circle((items[i][0], items[i][1]), 10, color=fillcolor,fill=True, alpha = 0.3)	
	circle2 = plt.Circle((items[i][0], items[i][1]), 20, color=fillcolor,fill=True, alpha = 0.3)	
